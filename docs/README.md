...
> **애플리케이션(산책로 탐색)을 찾는다면 → `../app/docs/`**
> 탐색 루프·로드뷰 수집·프롬프트 내용은 전부 그쪽이다. 같은 레포에 있지만
> HTTP 경계 하나로만 만난다. 여기 문서는 **판정 한 건**의 관심사만 다룬다.
...
| `07-batching-baseline.md` | 배칭 수정 실험 전 기준선. 구간 분해 원시 데이터, 배치 인코딩 프로브 |
...
- ✅ 애플리케이션(`../app/`) 배선 완료. 클라이언트 가이드의 규칙이 실제로 구현됨.
  실측 재확인: 콜드 스타트 13.0s → 정상 2.1s, `completion_tokens` 17(tiny)
