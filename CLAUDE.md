...
**`main` 에는 직접 못 넣는다.** GitHub 룰셋으로 PR 필수이고 관리자도 예외가
없다. 브랜치를 만들고 `gh pr create` 로 연다. 승인은 0건이라 본인이 바로
머지할 수 있다.

`gh pr create` 도 훅이 가로챈다 — 별도 에이전트가 **이 저장소를 처음 보는
사람**으로서 본문을 읽고, 가리키는 곳이 없는 참조나 diff 와 어긋나는 서술이
있으면 막는다 (→ `docs/12-harness.md` §4). PR 본문은 대화 맥락이 없는 사람이
읽는다는 것이 이 게이트의 전부다.
