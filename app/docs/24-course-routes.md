...
- 경유지 쌍당 요청 1회 + 3초 대기. 파일럿(종로 9코스) 총 ~45회.
- 응답 원본은 `app/labels/jongno/routes/{course}_{i:02d}.json` 에 캐시 —
  재실행은 네트워크에 나가지 않는다. 강제 재수집 = 캐시 파일 삭제.
- 수동 트레이스(길찾기가 실경로를 못 잇는 구간)는 같은 디렉터리에
  `{"source": "manual", ...}` 로 넣고 출처를 남긴다.
