...
이웃 panoId · **정확한 방위각**(화면 라벨의 8방위 버킷이 아니라 소수) · WGS84 좌표 ·
...
포기하고, walk.py 는 좌표 밀기로 되돌아간다. → `23-open-questions.md` §7

#### 폴백은 남겨둔다

이웃을 못 주는 provider 를 위해 예전 방식(좌표를 heading 방향으로 밀고
`getNearestPanoId` 로 스냅)이 그대로 살아 있다. 두 방식이 같은 판정 루프를
공유한다 → `20-app-design.md` §3.
