...
없다. 스마트서울맵(map.seoul.go.kr)이 코스 선을 실제로 그리므로 거기서 꺼내면
되겠지만 페이로드 형식을 못 알아냈다.

**폴리라인은 다른 길로 확보했다** (2026-08-19): 경유지 이름을 지오코딩해
카카오 도보 길찾기로 잇는다 → app/docs/22-labels.md §7, 24-course-routes.md.
스마트서울맵은 "코스 위 어디까지 커버했나" 를 재려면 여전히 필요하다.
